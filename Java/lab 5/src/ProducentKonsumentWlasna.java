...
public class ProducentKonsumentWlasna{
	...
	{
		Thread watek = new Thread(new Okno());
		watek.start();
...
class Okno extends JFrame implements Runnable, ActionListener
{
	...
	public void run()
	{
		...
		while(true)
		{
		//pSystem.out.println(timer.ileMiliSekundMineloTeraz());
			Date czas2 = new Date();
			long time = (czas2.getTime() - czas.getTime());
			if(time > 100)
			{
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		{
			if (przyciskZatrzymajWznow.getText() == "Stop")
			{
				try {
